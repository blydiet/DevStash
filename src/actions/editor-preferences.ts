...
export async function updateEditorPreferences(
  ...
  try {
    ...
    // Also catches requireSession() itself throwing (e.g. a session decrypt failure) —
    // this is a Server Action, so it must return a result rather than let an exception
    // cross the client/server boundary uncaught.
